...
namespace HeartBleed.Net
{
    ...
    public class HeartBleedTests
    {
        ...
        {
            int bytesPerLine = 16;
            string hexDump = buffer.Select((c, i) => new { Char = c, Chunk = i / bytesPerLine })
                .GroupBy(c => c.Chunk)
                .Select(g => g.Select(c => String.Format("{0:X2} ", c.Char)).Aggregate((s, i) => s + i))
        ...
        public byte[] GetHello(SSLVersion version)
        {
            byte[] data = {
        ...
        public byte[] GetHeartBeat(SSLVersion version)
        {
